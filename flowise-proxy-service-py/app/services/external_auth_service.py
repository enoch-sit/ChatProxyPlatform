...
class ExternalAuthService:
    ...
    async def get_user_by_username(self, username: str, admin_token: str) -> Optional[Dict]:
        """
        Get user details from external auth API by username.
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    f"{self.auth_url}/api/admin/users/by-username/{encoded_username}",
                    headers=headers,
                    timeout=self.timeout,
                )
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"User with username '{username}' not found in external auth system"
                    )
                    return None
                elif response.status_code == 401:
                    logger.warning("Unauthorized: Invalid or expired admin token")
            ...
            logger.error(f"Error fetching user by username from external auth: {e}")
            return None
